 ...
 */
class Mreview extends \Opencart\System\Engine\Controller {
	/**
	 * Index
	 *
	 * @return void
    ...
    public function install(): void
    {
        ...
        $this->model_setting_event->addEvent([
            'code' => "review_admin_panel",
            'description' => 'Add review search engine',
            'trigger' => 'admin/view/catalog/product_form/before',
            'action' => "extension/opencart/module/mreview.addReview",
            'status' => 1,
            'sort_order' => 0
        ]);
    }
    ...
    public function uninstall(): void {
        $this->model_setting_event->deleteEventByCode('review_admin_panel');
    }

    public function addReview(string $route, array &$data): void
    {
        // Событие которое отрабатывает перед рендерингом шаблона
        $product_id = $this->request->get['product_id'];

        $data['product_reviews'] = [];

        $this->load->model('extension/opencart/module/mreview');
        $product_reviews  = $this->model_extension_opencart_module_mreview->getProductReview($product_id);
        $data['total_review'] = $this->model_extension_opencart_module_mreview->getTotalProductReview($product_id);
        ...
        foreach ($product_reviews as $product_review) {
            $data['product_reviews'][] = [
                ...
                'date_added' => $product_review['date_added'],
            ];
        }
    }
}
